...
* AI 6-Stem 오디오 분리: 최첨단 AI 모델(Demucs)을 통해 하나의 오디오 파일을 보컬, 기타, 베이스, 피아노, 드럼, 기타 악기 총 6개의 트랙으로 분리합니다.
* 스템 믹서 (Stem Mixer): 분리된 트랙을 개별적으로 미리 들어보고 원하는 악기 파트만 선택할 수 있습니다.
...
3. FFmpeg (오디오 처리용)
   ...
   * Windows: FFmpeg 홈페이지에서 다운로드 후 환경변수 등록
...
3. 최초 실행 시 AI 모델을 다운로드하며, 이후 음원 분리에 약 1~3분이 소요됩니다.
